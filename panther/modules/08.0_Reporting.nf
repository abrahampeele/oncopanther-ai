...
# Reads output from 07.3_AcmgClassify.nf (patient_id_acmg.tsv + _acmg_summary.json)
# Falls back to VCF-based keyword scan if ACMG TSV not found
##################################################################################

ACMG_COLORS = {
    'Pathogenic':           '#E74C3C',   # red
    'Likely Pathogenic':    '#E67E22',   # orange
    'Uncertain Significance':'#F1C40F',  # yellow
    'Likely Benign':        '#2ECC71',   # light green
    'Benign':               '#27AE60',   # green
}
ACMG_BG = {
    'Pathogenic':           '#FADBD8',
...
CRITERIA_WEIGHTS = {
    'PVS1': 'Very Strong (P)', 'PS1': 'Strong (P)', 'PS2': 'Strong (P)',
    'PM1': 'Moderate (P)', 'PM2': 'Moderate (P)', 'PM4': 'Moderate (P)',
    'PM5': 'Moderate (P)', 'PP2': 'Supporting (P)', 'PP3': 'Supporting (P)',
    'PP5': 'Supporting (P)', 'BA1': 'Stand-Alone (B)', 'BS1': 'Strong (B)',
    'BP1': 'Supporting (B)', 'BP4': 'Supporting (B)', 'BP7': 'Supporting (B)',
}

def load_acmg_tsv(sample_id):
    '''Load ACMG TSV produced by 07.3_AcmgClassify.nf'''
    import glob, os
    candidates = (
        glob.glob(f"{sample_id}_acmg.tsv") +
        glob.glob(f"../**/{sample_id}_acmg.tsv", recursive=True) +
        glob.glob(f"*_acmg.tsv")
    )
    if not candidates:
        return None, None
    tsv = candidates[0]
    ...
    json_path = tsv.replace("_acmg.tsv", "_acmg_summary.json")
    summary = {}
...
def draw_acmg_classification_page(c, width, height, acmg_df, acmg_summary,
                                   metadata, oncopantherlogo):
    '''Draw a dedicated ACMG/AMP Classification page in the PDF report.'''
    c.showPage()
    draw_header_to_footer(c, width, height, metadata, oncopantherlogo)
    ...
    c.setFillColor(colors.HexColor('#2C3E50'))
    c.drawString(50, y, "ACMG/AMP Variant Classification")
    y -= 16
    c.setFont("Helvetica", 8)
    c.setFillColor(colors.HexColor('#7f8c8d'))
    c.drawString(50, y, "Richards et al. (2015) Genetics in Medicine 17:405-424  |  Criteria: PVS1, PS1, PM1-5, PP2-3,5, BA1, BS1, BP1,4,7")
    ...
    tiers = ['Pathogenic', 'Likely Pathogenic', 'Uncertain Significance', 'Likely Benign', 'Benign']
    counts = {}
    if acmg_summary and 'classification_counts' in acmg_summary:
        cc = acmg_summary['classification_counts']
        counts = {
            'Pathogenic':            cc.get('Pathogenic', 0),
            'Likely Pathogenic':     cc.get('Likely Pathogenic', 0),
            'Uncertain Significance':cc.get('Uncertain Significance', 0),
            'Likely Benign':         cc.get('Likely Benign', 0),
            'Benign':                cc.get('Benign', 0),
        }
    elif acmg_df is not None:
        vc = acmg_df['ACMG_CLASS'].value_counts().to_dict()
        counts = {t: vc.get(t, 0) for t in tiers}
    else:
    ...
    c.setFillColor(colors.HexColor('#2C3E50'))
    c.drawString(50, y, "Classification Summary")
    y -= 14

    summary_data = [['Tier', 'Count', '% of Total', 'Clinical Significance']]
    sig_map = {
        'Pathogenic':            'Disease-causing — immediate clinical action',
        'Likely Pathogenic':     'Likely disease-causing — clinical follow-up recommended',
        'Uncertain Significance':'Insufficient evidence — family studies advised',
        'Likely Benign':         'Likely not disease-causing — low clinical priority',
        'Benign':                'Not disease-causing — no clinical action needed',
    }
    for tier in tiers:
        cnt = counts.get(tier, 0)
        pct = f"{(cnt/total*100):.1f}%" if total > 0 else "0%"
        summary_data.append([tier, str(cnt), pct, sig_map.get(tier, '')])

    col_widths = [120, 40, 55, 270]
    sum_table = Table(summary_data, colWidths=col_widths)
    row_bgs = [colors.HexColor(ACMG_BG.get(row[0], '#FFFFFF')) for row in summary_data[1:]]
    sum_table.setStyle(TableStyle([
        ...
        # Colour the tier name cell itself
        *[('TEXTCOLOR',  (0,i+1),(0,i+1), colors.HexColor(ACMG_COLORS.get(summary_data[i+1][0],'#2C3E50')))
          for i in range(len(tiers))],
        *[('FONTNAME',   (0,i+1),(0,i+1), 'Helvetica-Bold') for i in range(len(tiers))],
    ]))
    tbl_h = len(summary_data) * 16
    sum_table.wrapOn(c, width - 100, 400)
    sum_table.drawOn(c, 50, y - tbl_h)
    y -= tbl_h + 20

    # ── Top Pathogenic / Likely Pathogenic Variants Table ────────────────────
    if acmg_df is not None:
        path_df = acmg_df[acmg_df['ACMG_CLASS'].isin(['Pathogenic', 'Likely Pathogenic'])]
        if not path_df.empty:
            c.setFont("Helvetica-Bold", 10)
            c.setFillColor(colors.HexColor('#2C3E50'))
            c.drawString(50, y, f"Actionable Variants  (Pathogenic / Likely Pathogenic)  — Top {min(15, len(path_df))}")
            y -= 14
            rows_to_show = path_df.head(15)
            var_data = [['Gene', 'HGVSc', 'Consequence', 'Impact', 'gnomAD_AF', 'ClinVar', 'Criteria', 'Classification']]
            for _, row in rows_to_show.iterrows():
                var_data.append([
                    str(row.get('GENE', '.')),
                    str(row.get('HGVSc', '.'))[:30],
                    str(row.get('CONSEQUENCE', '.'))[:22],
                    str(row.get('IMPACT', '.')),
                    str(row.get('gnomAD_AF', '.')),
                    str(row.get('ClinVar_CLNSIG', '.'))[:14],
                    str(row.get('CRITERIA', 'None'))[:18],
                    str(row.get('ACMG_CLASS', '.')),
                ])
            col_w = [48, 90, 80, 45, 52, 55, 65, 80]
            var_table = Table(var_data, colWidths=col_w)
            var_row_bgs = [
                colors.HexColor(ACMG_BG.get(r[-1], '#FFFFFF'))
                for r in var_data[1:]
            ]
            var_table.setStyle(TableStyle([
                ...
                ('ROWBACKGROUNDS',(0,1), (-1,-1), var_row_bgs),
                ('ROWPADDING',    (0,0), (-1,-1), 3),
                ('WORDWRAP',      (0,0), (-1,-1), True),
            ]))
            var_h = len(var_data) * 13
            if y - var_h < 60:   # new page if needed
                c.showPage()
                draw_header_to_footer(c, width, height, metadata, oncopantherlogo)
                y = height - 90
            var_table.wrapOn(c, width - 100, 600)
            var_table.drawOn(c, 50, y - var_h)
            y -= var_h + 20
        ...
        c.setFont("Helvetica-Bold", 9)
        c.setFillColor(colors.HexColor('#2C3E50'))
        c.drawString(50, y, "ACMG/AMP Criteria Legend")
        y -= 12
        c.setFont("Helvetica", 7)
        c.setFillColor(colors.HexColor('#555555'))
        legend_items = list(CRITERIA_WEIGHTS.items())
        cols = 3
        col_w = (width - 100) / cols
        for idx, (crit, desc) in enumerate(legend_items):
            cx = 50 + (idx % cols) * col_w
            cy = y - (idx // cols) * 11
            c.setFont("Helvetica-Bold", 7)
            c.setFillColor(colors.HexColor('#E74C3C') if '(P)' in desc else colors.HexColor('#27AE60'))
            c.drawString(cx, cy, f"{crit}:")
            c.setFont("Helvetica", 7)
            c.setFillColor(colors.HexColor('#555555'))
            c.drawString(cx + 28, cy, desc)
        y -= (len(legend_items) // cols + 1) * 11 + 10

        # ── Framework note ────────────────────────────────────────────────────
        c.setFont("Helvetica-Oblique", 6.5)
        c.setFillColor(colors.HexColor('#7f8c8d'))
        c.drawString(50, y,
            "Classification based on ACMG/AMP 2015 guidelines. VEP --everything --check_existing. "
            "Criteria evaluated: PVS1, PS1, PM1-5, PP2-3,5, BA1, BS1, BP1,4,7. "
            "Requires clinical validation before therapeutic decisions."
        )

# ── Load ACMG results and draw page ──────────────────────────────────────────
acmg_df, acmg_summary = load_acmg_tsv("${metadata.SampleID}")
draw_acmg_classification_page(c, width, height, acmg_df, acmg_summary, metadata, oncopantherlogo)

# Legacy summary table on original page (still drawn for backwards compat)
def draw_acmg_table(c, df, x, y):
    counts = {}
    if df is not None and 'ACMG_CLASS' in df.columns:
        counts = df['ACMG_CLASS'].value_counts().to_dict()
    tiers = ['Pathogenic','Likely Pathogenic','Uncertain Significance','Likely Benign','Benign']
    data = [['ACMG Tier', 'Count']] + [[t, str(counts.get(t,0))] for t in tiers]
    row_colors = [colors.HexColor(ACMG_BG.get(r[0],'#FFFFFF')) for r in data[1:]]
    style = TableStyle([
        ('BACKGROUND', (0,0),(-1,0),  colors.HexColor('#2C3E50')),
        ('TEXTCOLOR',  (0,0),(-1,0),  colors.white),
...
draw_acmg_table(c, acmg_df, 440, 234)
